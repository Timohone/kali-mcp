...
class KaliToolsServer:
    ...
    def load_tools_data(self):
        ...
        if not self.kali_tools_path.exists():
            print(
                f"ERROR: Kali tools directory not found: {self.kali_tools_path}",
                file=sys.stderr,
            )
        ...
        for item in all_items[:10]:  # Show first 10 items
            print(
                f"  {item.name} ({'dir' if item.is_dir() else 'file'})", file=sys.stderr
            )
        ...
        for i, md_file in enumerate(markdown_files[:5]):
            print(
                f"  Example {i+1}: {md_file.relative_to(self.kali_tools_path)}",
                file=sys.stderr,
            )
        ...
        print(
            f"Found {len(tool_dirs)} directories with markdown files", file=sys.stderr
        )
        ...
        for md_file in markdown_files:
            try:
                ...
                if (
                    md_file.name.lower() in ["readme.md", "_index.md"]
                    and md_file.parent == self.kali_tools_path
                ):
                ...
                if len(self.tools_data) < 5:
                    lines = content.split("\n")[:5]
                    print(
                        f"File {md_file.relative_to(self.kali_tools_path)} first lines:",
                        file=sys.stderr,
                    )
                    for line in lines:
                ...
                if tool_name and tool_name not in [
                    ...
                    self.tools_data[tool_name] = {
                        ...
                        "category": self.guess_category_from_content(
                            content, tool_name
                        ),
                    }
                    ...
                    if len(self.tools_data) <= 5:  # Show first 5 successful parses
                        print(
                            f"Successfully parsed tool: {tool_name} from {md_file.relative_to(self.kali_tools_path)}",
                            file=sys.stderr,
                        )
                else:
                    if len(self.tools_data) < 10:  # Only show first few failures
                        print(
                            f"Skipped or could not extract tool name from: {md_file.relative_to(self.kali_tools_path)} (extracted: '{tool_name}')",
                            file=sys.stderr,
                        )
        ...
        print(
            f"Final result: Loaded {len(self.tools_data)} Kali tools", file=sys.stderr
        )
        ...
        if self.tools_data:
            ...
            for i, (tool_name, data) in enumerate(list(self.tools_data.items())[:5]):
                print(
                    f"  {tool_name}: {data.get('description', 'No description')[:50]}...",
                    file=sys.stderr,
                )
        else:
    ...
    def extract_tool_name(self, md_file: Path, content: str) -> str:
        ...
        if verbose:
            print(
                f"Extracting tool name from: {md_file.relative_to(self.kali_tools_path)}",
                file=sys.stderr,
            )
                ...
                if len(parts) >= 3:
                    ...
                    if metadata and "title" in metadata:
                        ...
                        if tool_name not in [
                            ...
                            if verbose:
                                print(
                                    f"  Found in YAML frontmatter: {tool_name}",
                                    file=sys.stderr,
                                )
                            return tool_name
            except Exception as e:
        ...
        if md_file.parent != self.kali_tools_path:
            ...
            if dir_name not in ["tools", "documentation", "kali-tools", "all-tools"]:
                ...
                if clean_dir_name:
                    if verbose:
                        print(
                            f"  Using directory name: {clean_dir_name}", file=sys.stderr
                        )
    ...
    def parse_markdown_content(self, content: str) -> Dict[str, Any]:
        ...
        for i, line in enumerate(clean_lines):
            ...
            if (
                "packagesinfo:" in line_stripped.lower()
                or "packages info:" in line_stripped.lower()
            ):
            ...
            if line_stripped.startswith("```"):
                ...
                if current_section and any(
                    ...
                    for keyword in ["usage", "example", "command"]
                ):
            ...
            if (
                "usage example" in line_stripped.lower()
                or "example usage" in line_stripped.lower()
            ):
            ...
            if current_section:
                if any(
                    keyword in current_section
                    for keyword in ["usage", "example", "command", "syntax", "how to"]
                ):
                    usage_content.append(line)
                elif any(
                    keyword in current_section
                    for keyword in ["install", "setup", "apt", "dependencies"]
                ):
                ...
                if (
                    ...
                    and not any(
                        skip in line_stripped.lower()
                        for skip in ["usage example", "screenshot", "video"]
                    )
                ):
                    desc_content.append(line_stripped)
        ...
        if not data["description"]:
            ...
            if debug_this:
                ...
                print(
                    f"  YAML data: {yaml_data.get('description', 'None')}",
                    file=sys.stderr,
                )
            ...
            if "description" in yaml_data and yaml_data["description"]:
                ...
                if debug_this:
                    print(
                        f"  Used YAML description: {data['description'][:50]}...",
                        file=sys.stderr,
                    )
            # Try collected description content
                ...
                description_text = re.sub(
                    r"\[([^\]]+)\]\([^\)]+\)", r"\1", description_text
                )  # Remove markdown links
                description_text = re.sub(
                    r"[*_`]", "", description_text
                )  # Remove formatting
                description_text = re.sub(
                ...
                if debug_this:
                    print(
                        f"  Used desc content: {data['description'][:50]}...",
                        file=sys.stderr,
                    )
            # Try first meaningful paragraph after cleaning
                ...
                for i, line in enumerate(
                    clean_lines[:20]
                ):  # Check first 20 clean lines
                    line = line.strip()
                    if (
                        not line.startswith("#")
                        and len(line) > 20
                        and not line.startswith("```")
                        and not line.startswith("{{")  # Skip remaining Hugo syntax
                        and not line.startswith("![")  # Skip images
                        and not any(
                            skip in line.lower()
                            for skip in [
                                "screenshot",
                        ...
                        if len(clean_desc) > 20:
                            ...
                            if debug_this:
                                print(
                                    f"  Used line {i}: {data['description'][:50]}...",
                                    file=sys.stderr,
                                )
                            break
            ...
            if not data["description"] and "title" in yaml_data:
                ...
                if (
                    title
                    and title.lower() not in ["kali tools", "all kali tools"]
                    and len(title) > 2
                ):
                    ...
                    if debug_this:
                        print(
                            f"  Used title fallback: {data['description']}",
                            file=sys.stderr,
                        )
        ...
        if not data["homepage"] or not data["repository"]:
            ...
            for url in urls:
                if (
                    ("github.com" in url or "gitlab.com" in url)
                    and "/tree/" not in url
                    and "/blob/" not in url
                ):
                    ...
                    and "http" in url
                    and not any(
                        skip in url for skip in ["kali.org/tools", "example.com"]
                    )
                ):
            ...
            if name == "search_kali_tools":
                ...
                if tool_name not in self.tools_data:
                    return [
                        TextContent(
                            type="text",
                            text=f"Tool '{tool_name}' not found in Kali tools database.",
                        )
                    ]
                ...
                if category not in self.categories:
                    return [
                        TextContent(
                            type="text", text=f"Category '{category}' not found."
                        )
                    ]
                ...
                if tool_name not in self.tools_data:
                    return [
                        TextContent(type="text", text=f"Tool '{tool_name}' not found.")
                    ]
